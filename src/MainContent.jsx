...
const MainContent = () => {
    ...
    return (
        <main className="main-content w-full flex-1 flex flex-col mt-4 gap-4 text-2xl">
            <section id="welcome" className="w-full mb-8"></section>
            <section id="about" className="w-full flex flex-col gap-4 mb-8">
                <h1 className="text-5xl font-bold mb-4 font-teko tracking-wider">About Me:</h1>
                <div className="flex xsm:flex-col md:flex-row xsm:items-center justify-center xsm:gap-4 md:gap-24">
                    ...
                    <div className="flex flex-col gap-6 md:w-1/2 text-2xl">
                        <p className=" leading-10">Hello, and welcome to justincare.io!</p>
                        <p className=" leading-10">I'm Justin, and I've spent the last decade+ as a Frontend Developer of various stripes.</p>
                        <p className=" leading-10">This is a place for me to house and display various projects that I'm working/worked on. A sort of digital portfolio and resume if you will.</p>
                        <p className=" leading-10">You can find me on my <button className="mx-2 rounded-sm bg-blue-500 hover:bg-blue-200 focus:bg-blue-200 text-slate-100 hover:text-slate-800 focus:text-slate-800 px-2 pb-2" tabIndex={-1}><a href={urls.github} target="_blank">Github<FaGithub className="inline-block text-3xl ml-2" /></a></button> and <button className="mx-2 rounded-sm bg-blue-500 hover:bg-blue-200 focus:bg-blue-200 text-slate-100 hover:text-slate-800 focus:text-slate-800 px-2 pb-2" tabIndex={-1}><a href={urls.linkedin} target="_blank">LinkedIn<FaLinkedin className="inline-block text-3xl ml-2" /></a></button> profiles.</p>
                        <p className=" leading-10">And, if by chance, you accidentally came to this site looking for my writing/books, you can head over to <button className="mx-2 rounded-sm bg-blue-500 hover:bg-blue-200 focus:bg-blue-200 text-slate-100 hover:text-slate-800 focus:text-slate-800 px-2 pb-2 items-center" tabIndex={-1} target="_blank"><a href={urls.personal} target="_blank">my other site<FaBook className="inline-block text-3xl ml-2" /></a></button>.</p>
                    </div>
                </div>
            </section>
            <section id="resume" className="w-full min-h-80 flex flex-col gap-4">
                <h1 className="text-5xl font-bold font-teko tracking-wider">The short, TLDR; version:</h1>
                <p className=" leading-10">I worked as an "Interactive Developer" for the same company for almost 13 years.</p>
                <span className=" leading-10 italic">What is an Interactive Developer you ask?</span>
                <p className=" leading-10">Essentially, I was a specialized, singular Frontend Developer on a media team. I worked on a variety of projects, from VR/AR prototypes, to simulations, all the way down to your basic "<span className="italic">click a series of images to view related text and images in a modal</span>" kind of thing.</p>
                <p className=" leading-10 mb-6">That being said, I'm not able to display most or any of that work, given it was all internal, for a former employer. In lieu of that, I'll be working on new projects that I can share on this site. If you'd like to view my detailed resume, click <ResumeButton /> to download the file, or keep scrolling to view it here.</p>
                <Resume />
            </section>
            <section id="projects" className="w-full min-h-80 flex flex-col my-4 mb-14">
                <h1 className="text-5xl font-bold mb-6 mt-8 font-teko tracking-wider">Projects:</h1>
                <p className=" leading-10 mb-6 xsm:mb-10">This is a "coming soon" kind of list. I'm currently working on some personal projects, and will update here with more details when they are ready. For now, here are some of the projects I have on GitHub right now (mainly just this site right now 👀 😮‍💨):</p>
                ...
                ResumeItems.map((item, index) => {
                    return (
                        <Disclosure key={index}>
                            <DisclosureButton data-index={index} onClick={handleAccOpen} onKeyDown={handleAccOpen} className="flex flex-row items-center gap-2  cursor-pointer mb-2 dark:text-slate-100 w-full">
                                <div className="flex flex-row justify-between items-center gap-2 pointer-events-none w-full">
                                    <span className="pointer-events-none font-bold text-4xl font-teko tracking-widest">{item.title}</span>
                                    {
